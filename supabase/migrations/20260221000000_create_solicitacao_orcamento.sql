-- Tabela para solicitações de orçamento (estilo GetNinjas)
-- Execute no Supabase Dashboard > SQL Editor se ainda não existir
...
-- RLS: usuário autenticado pode inserir sua própria solicitação e ver as suas
...
CREATE POLICY "Users can insert own solicitacao"
  ON solicitacao_orcamento FOR INSERT
  TO authenticated
...
CREATE POLICY "Users can view own solicitacoes"
  ON solicitacao_orcamento FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

COMMENT ON TABLE solicitacao_orcamento IS 'Solicitações de orçamento do cidadão; empresas podem ser notificadas ou consultar por categoria/cidade (políticas futuras).';
