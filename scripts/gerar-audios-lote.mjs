...
async function main() {
  ...
  for (let i = 0; i < noticias.length; i += BATCH_SIZE) {
    ...
    const promises = lote.map(async (noticia, index) => {
      ...
      const tituloTruncado = noticia.titulo.length > 50
        ? noticia.titulo.slice(0, 50) + '...'
        : noticia.titulo;

      console.log(`${numero}/${noticias.length} - Gerando: ${tituloTruncado}`);

      const resultado = await gerarAudio(noticia.id, noticia.titulo);

      if (resultado) {
        const status = resultado.cached ? '[CACHE]' : '[NOVO]';
        const arquivo = resultado.audioUrl.split('/').pop();
        console.log(`  ✅ Sucesso: ${status} ${arquivo}`);
        sucesso++;
      } else {
        erros++;
      }
